...
export default function AuditResultsPoller({ sessionId }: { sessionId: string }) {
  ...
  if (networkError && !payload) {
    return (
      <div className="mt-8 rounded-lg border border-red-400/40 bg-red-500/10 p-6">
      ...
      <div className="mt-8 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <a
            ...
            className="inline-flex justify-center rounded-md bg-[var(--color-accent)] px-6 py-3 text-base font-semibold text-black transition hover:brightness-110"
          >
            Download PDF audit
          </a>
          <a
            href={payload.json_url}
            className="inline-flex justify-center rounded-md border border-[var(--color-border)] px-6 py-3 text-base text-[var(--color-fg)] transition hover:bg-[var(--color-surface)]"
          >
    ...
    return (
      <div className="mt-8 rounded-lg border border-red-400/40 bg-red-500/10 p-6">
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row">
          <button
            onClick={onRetry}
            className="rounded-md border border-red-400/40 px-5 py-2 text-sm text-red-200 transition hover:bg-red-500/20"
          >
            Re-check status
          </button>
          <a
